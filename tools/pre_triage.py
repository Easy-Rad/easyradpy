...
def get_reports(connection: psycopg.Connection):
    with connection.cursor() as cur:
        cur.execute("""
            select pa_nhi, ce_description, re_dor, te_text
            from doctext
                join reports on te_key=re_serial and te_key_type='R'
                join case_event on re_event_serial=ce_serial
                join case_main on ce_cs_serial=cs_serial
                join patient on cs_pno=pa_pno
            order by te_serial desc
            limit 50
        """)
        return cur.fetchall()
    # todo optimise with indexed query
